...
from quantumtrader.models import Order, Position, RiskDecision


class RiskManager:
    """Central risk controller validating all trades before execution."""
    ...
    def should_halt(self, equity: float, start_of_day_equity: float, peak_equity: float) -> str | None:
        """Return halt reason if daily loss or drawdown limits are breached."""
    ...
    def position_size(self, equity: float, atr_value: float, price: float) -> float:
        """Kelly-style sizing: (1% equity) / (2 × ATR), capped at 25% portfolio."""
    ...
    def validate_order(
        ...
        if sector:
            sectors = Counter(
                position.sector for position in positions if position.sector
            )
            if sectors[sector] >= self.max_assets_per_sector:
